<div class="spectrumLayer">
	<div class="data">
		<div class="spectrumJoinLayer">
			...
			<div class="formArea">
<form name="fwrite7" method="post" action="/board/index.php?board=member&type=action_member&skin=root" accept-charset="UTF-8" onsubmit="return false;" enctype="multipart/form-data">
	...
	<input type="hidden" name="drop" value="drop||pw||board||skin||url||prikey||editor_data||file_save_type||x||y||inquiry_action||pw_next">
	<div id="goto_content" class="viewTop">
		<table class="view_table" summary="이 표는 글쓰기 기능을 하며 글쓴이, 비밀번호, 이메일, 제목, 내용, 첨부파일로 구성되어 있습니다.">
			...
			<tbody>
				...
				<tr>
					<th scope="col"><span>*</span> <label for="nick">가맹점명</label></th>
					<td class="writeTitle">
...
-->
	...
	<textarea class="script">
		$('.addr_search').click(function(e){
			window.open('/admin/view3_map_00/postcode.php?fname=fwrite7&geocode=off', 'postcode', 'width=500, height=600, left=150, top=100');
		});
		function view3_submits(f) {
		...
		window.reg_mb_id_check = function() {
			$.ajax({
				type: 'POST',
				url: '/admin/view3_member_00/check.php',
				data: {
		...
		$('.spectrumJoinAction').click(function(){
			if(!view3_submits($('[name="fwrite7"]')[0])){
				return false;
			}
			...
			$.ajax({
				...
				success : function(response){
					...
					if(request=='ok'){
						spectrumAlert("가입신청이 완료되었습니다.\n확인 후 결과를 통보 해드리겠습니다");
						setTimeout(function(){
							$('.spectrum_close').trigger('click');
						},3500);
					}else{
						if(request!==''){
