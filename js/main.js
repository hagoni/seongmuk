...
(function($) {
	$(document).ready(function() {
		TweenMax.to($('.flow_in'), 30.0, {backgroundPositionY: "-1200px", ease: Power0.easeNone, repeat: -1});
	});
}(jQuery));
(function($){
	...
	$(window).resize(function(){
		...
		if (documentWidth < 768) {
			if ($('.news_03').hasClass('on')) {
				$('.news_03').removeClass('on');
			}
		} else {
			if (!$('.news_03').hasClass('on')) {
				$('.news_03').addClass('on');
...
(function($) {
	$(document).ready(function() {
		...
		window.section01_SwiperBanner = new SwiperBannerMotion(
			...
			,[
				[
					...
					{el: '.mv01_box_wrap', set: {opacity: 0}, to: {opacity: 1}, d: 0.6},
				],
				[
		...
		window.section02_SwiperBanner = new SwiperBannerMotion(
			'.brand > .swiper-container'
			,[
				[
					{el: '.br4_icon1', set: {opacity:0, y: -120,x: 0}, to: {opacity: 1, y: 0, x: 0, ease: Expo.easeOut}, d: 0.6, t: '+=0.3'},
					{el: '.icon2_1', set: {opacity:0, y: -60}, to: {opacity: 1, y: 0, ease: Expo.easeOut}, d: 0.2, t: '+=0.1'},
					{el: '.icon3', set: {opacity:0, y: -60}, to: {opacity: 1, y: 0, ease: Expo.easeOut}, d: 0.2, t: '-=0.1'},
					{el: '.icon4', set: {opacity:0, y: -60}, to: {opacity: 1, y: 0, ease: Expo.easeOut}, d: 0.2, t: '-=0.1'},
					{el: '.icon5', set: {opacity:0, y: -60}, to: {opacity: 1, y: 0, ease: Expo.easeOut}, d: 0.2, t: '-=0.1'},
					{el: '.icon6', set: {opacity:0, y: -60}, to: {opacity: 1, y: 0, ease: Expo.easeOut}, d: 0.2, t: '-=0.1'},
					{el: '.icon2_2', set: {opacity:0}, to: {opacity: 1}, d: 0.3, t: '+=0.3'},
					{el: '.brin_slide_m', set: {opacity:0}, to: {opacity: 1, y: 0, ease: Expo.easeIn}, d: 0.3, t: '0'},

				],
				[
					{el: '.icon1', set: {opacity:0, y: -120,x: 120}, to: {opacity: 1, y: 0, x: 0, ease: Expo.easeOut}, d: 0.6, t: '+=0.3'},
					{el: '.icon2', set: {opacity:0, y: -200}, to: {opacity: 1, y: 0, ease: Expo.easeIn}, d: 0.6, t: '+=0.1'},
					{el: '.brin_slide_m', set: {opacity:0}, to: {opacity: 1, y: 0, ease: Expo.easeIn}, d: 0.3, t: '0'},
				],
				[
					{el: '.icon1', set: {opacity:0, y: -120,x: 120}, to: {opacity: 1, y: 0, x: 0, ease: Expo.easeOut}, d: 0.6, t: '+=0.3'},
		...
		section02_SwiperBanner.setChangeCallbackBefore(function(idx,slideWrap){
		});
		section02_SwiperBanner.setChangeCallbackAfter(function(idx,slideWrap){
			if(typeof $('.brin_slide_m .swiper-container', slideWrap).swiper === 'object'){
				$('.brin_slide_m .swiper-container', slideWrap).swiper.destroy();
			}
			var brinSwiper = new Swiper($('.brin_slide_m .swiper-container', slideWrap)[0], {
				slidesPerView: 'auto',
				spaceBetween: 60,
        ...
        window.fullPageMotion = new FPMotion('fullpage', [
            [
                {method: 'call', fx: function() {
					section01_SwiperBanner.play();
					section01_SwiperBanner.getSwiper().update();
                }},
                {method: 'onPause', fx: function() {
					section01_SwiperBanner.stop();
                }},
            ],
			[
                {method: 'call', fx: function() {
					section02_SwiperBanner.play();
                }},
                {method: 'onPause', fx: function() {
					section02_SwiperBanner.stop();
                }},
			],
			[
				{method: 'call', fx: function() {
