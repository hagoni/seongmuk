...
(function($) {
	...
	Geomap.prototype = {
		...
		opt: {
			initGeocode: {lat: 37.566535, lng: 126.977969},
			mapOptions: {
				zoom: 12,
	...
	Geomap.prototype.getCurrentPosition = function(callback, geocode) {
		if(window.location.protocol.indexOf('https') > -1 && this.checkMobile() === true) {
		 ...
		 */
		 this.updateList = function(query) {
 			$.get(listUrl, query, function(response) {
 				boardWrap.html($(response).find('#storeListWrap').html());
 			}, 'html').error(function(e) {
 				alert(e.statusText);
 			});;
 		};
